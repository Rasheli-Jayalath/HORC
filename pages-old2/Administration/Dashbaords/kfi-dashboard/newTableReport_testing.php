...
$eSqls = "Select * from project_currency ";
  ...
  $eeCount = $objDb->totalRecords();
    ...
    $itemid1 = $_GET['itemids'];
    $query = "Select parentgroup from boqdata where itemid=$itemid1";
    $objDb -> dbQuery($query);
    $rCount = $objDb->totalRecords();
    if($rCount > 0){
    ...
    $itemidcount = explode("_", $parentgroup);
     ...
     foreach ($itemidcount as $itemid) {
      $query2 = "Select itemname,itemid from boqdata where itemid=$itemid";
      $objDb -> dbQuery($query2);
      $rCount2 = $objDb->totalRecords();
      if($rCount2 > 0){
