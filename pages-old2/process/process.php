...
while($i<$count)
{
	$i++;
		...
		$tpresult =$objDb->dbQuery($tpsql);
		$tprows = $objDb->dbFetchArray();
		...
		$mpresult =$objDb->dbQuery($mpsql);
		$mprows = $objDb->dbFetchArray();
		...
		$apresult = $objDb->dbQuery($apsql);
		$aprows =$objDb->dbFetchArray();
