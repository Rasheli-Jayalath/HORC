...
if($saveBtn != "")
{
  ...
  $eCount = $objDb1->totalRecords();
   ...
   $sSQL = ("INSERT INTO boqdata (parentcd, activitylevel, stage,factor,itemcode, itemname,weight, isentry, aorder,lid) VALUES ($txtparentcd,$boqlevel+1,'$txtstage',$factor,'$txtitemcode', '$txtitemname',$weight,$txtisentry,$aorder,$lid )");
  }
  else
  {
	  $parentgroup2="";
	  $txtparentcd=0;
	  $sSQL = ("INSERT INTO boqdata (parentcd, activitylevel, stage,factor,itemcode, itemname,weight, isentry, aorder,lid) VALUES ($txtparentcd,$boqlevel,'$txtstage',$factor,'$txtitemcode', '$txtitemname',$weight,$txtisentry,$aorder,$lid)");
	}
	...
	$parentgroup1=str_repeat("0",$_SESSION['codelength']-strlen($itemids)).$itemids;
	 ...
	 $uSqlu = "Update boqdata SET 
			 ...
			 lid='$itemids' where itemid= $itemids";	
