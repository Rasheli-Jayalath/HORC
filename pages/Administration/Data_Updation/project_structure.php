...
$created_by	= $_SESSION['ne_fullname_name'] ;
	 $userid_owner	= $_SESSION['ne_user_cd'];
...
if(isset($_REQUEST['save']))
 {
	 ///BOQ Component Level Details///////////
	 $bSQL = "SELECT * FROM  boqdata where parentcd=0";
	 ...
	 ///maindata Component Level Details///////////
	  $bSQL_main = "SELECT * FROM  maindata where parentcd=0";
	 $pSQLResult_main = $objDb->dbQuery($bSQL_main);
	 $bData_main = $objDb->dbFetchArray();
	 $parentcd_main=$bData_main["itemid"];
	  $parentgroup1_main=$bData_main["parentgroup"];
	
	
	 ///KPI Component Level Details///////////
	 $bSQL_kpi = "SELECT * FROM  kpidata where parentcd=0";
	 $pSQLResult_kpi = $objDb->dbQuery($bSQL_kpi);
	 $bData_kpi = $objDb->dbFetchArray();
	...
	if ($sql_pro == TRUE) {
		$albumid=$objAdminUser->genCode("t031project_albums", "albumid");
		 $sql_pro0="INSERT INTO t031project_albums(albumid,pid, album_name, status,parent_album, creater, creater_id,lid) 
		Values(".$albumid.", ".$pid.", '".$album_name."' , ".$status.", '".$parent_album."','".$creater."' ,".$userid_owner." ,".$lid.")";
	$sql_pro=$objDb->dbQuery($sql_pro0);
	$albumid=$con->lastInsertId();
	...
	///////////////////////////Map & Drawing//////////////////////
		$albumid=$objAdminUser->genCode("t031project_drawingalbums", "albumid");
		 $sql_pro1="INSERT INTO t031project_drawingalbums(albumid,pid, album_name, status,parent_id, creater, creater_id,lid) 
		Values(".$albumid.", ".$pid.", '".$album_name."' , ".$status.", '".$parent_album."','".$creater."' ,".$userid_owner.", '".$lid."')";
		$sql_pro=$objDb->dbQuery($sql_pro1);
	$albumida=$con->lastInsertId();
	if($parent_album==0)
		{
		
			if(strlen($albumida)==1)
			{
			$parent_group="00".$albumida;
			}
			else if(strlen($albumida)==2)
			{
			$parent_group="0".$albumida;
			}
			else
			{
			$parent_group=$albumida;
			}
		}
	 $sql_pro="UPDATE t031project_drawingalbums SET parent_group='$parent_group' where albumid=$albumida";
	...
	/*///////////////////////////BOQ Entry//////////////////////
		...
		$parentcd=1;
		 ...
		 $sql_pro1="INSERT INTO boqdata(parentcd, parentgroup, activitylevel,stage,factor,itemcode,itemname,weight, isentry, aorder,lid) 
		Values( ".$parentcd.", '".$parentgroup1."' , '".$activitylevel."' , '".$stage."' , '".$factor."' , '".$itemcode."' , '".$itemname."' , '".$weight."' , '".$isentry."' , '".$aorder."' , '".$lid."')";
		$sql_pro=$objDb->dbQuery($sql_pro1);
	...
	$activitylevel=1;
		...
		$lid=0;
				...
				$sql_pro1="INSERT INTO maindata(parentcd, parentgroup, activitylevel,itemcode,itemname,weight, isentry, aorder,lid) 
		Values( ".$parentcd.",  '".$parentgroup1_main."',  '".$activitylevel."' , '".$itemcode."' , '".$itemname."' , '".$weight."' , '".$isentry."' , '".$aorder."' , ".$lid.")";
		$sql_pro=$objDb1->dbQuery($sql_pro1);
		$itemid_main=$con->lastInsertId();
		
		if($parentcd==1)
		{		
			if(strlen($itemid_main)==1)
			{
			$parent_group="00000".$itemid_main;
			}
			else if(strlen($itemid_main)==2)
			{
			$parent_group="0000".$itemid_main;
			}
			else if(strlen($itemid_main)==3)
			{
			$parent_group="000".$itemid_main;
			}
			else if(strlen($itemid_main)==4)
			{
			$parent_group="00".$itemid_main;
			}
			else if(strlen($itemid_main)==5)
			{
			$parent_group="0".$itemid_main;
			}
			else
			{
			$parent_group=$itemid_main;
			}
		}
		 $parent_group=$parentgroup1_main."_".$parent_group;
	$sql_pro="UPDATE maindata SET parentgroup='$parent_group' where itemid=$itemid_main";
	...
	//KPI Data start
	 	...
	 	$sql_protem="select * from  kpi_templates limit 0,1";
	$sql_pro_tem=$objDb3->dbQuery($sql_protem);
		 $bData_temp_kpi = $objDb3->dbFetchArray();
		 $kpi_temp_id=$bData_temp_kpi['kpi_temp_id'];
		...
		$lid=0;
				 ...
				 $sql_pro2="INSERT INTO kpidata(parentcd, parentgroup,  activitylevel,itemcode,itemname,weight, isentry, aorder,kpi_temp_id,lid) 
		Values( ".$parentcd.",'".$parentgroup1_kpi."',  '".$activitylevel."' , '".$itemcode."' , '".$itemname."' , '".$weight."' , '".$isentry."' , '".$aorder."', ".$kpi_temp_id." , ".$lid.")";
		$sql_pro2=$objDb2->dbQuery($sql_pro2);
		$itemid_kpi=$con->lastInsertId();
		
		if($parentcd==1)
		{		
			if(strlen($itemid_kpi)==1)
			{
			$parent_group="00000".$itemid_kpi;
			}
			else if(strlen($itemid_kpi)==2)
			{
			$parent_group="0000".$itemid_kpi;
			}
			else if(strlen($itemid_kpi)==3)
			{
			$parent_group="000".$itemid_kpi;
			}
			else if(strlen($itemid_kpi)==4)
			{
			$parent_group="00".$itemid_kpi;
			}
			else if(strlen($itemid_kpi)==5)
			{
			$parent_group="0".$itemid_kpi;
			}
			else
			{
			$parent_group=$itemid_kpi;
			}
		}
...
<body>
  ...
  <h3 style="text-align:center"><?php echo $message; ?></h3>
    ...
    <input   type="submit" name="update" id="update" value="<?php echo "Update";?>" class="btn bg-success m-auto text-white btn-sm" style="font-weight:500"/>
	 <?php
	 }
	 else
	 {
