  ...
  if(isset($_POST['submit']) && $_POST['level1']>0 ){
    ...
    $activitylevel = $row['activitylevel'];
        ...
        if($activitylevel==4){
          ...
          $sql .=  "SELECT * FROM maindata WHERE  parentcd = 0 OR parentgroup = '$aLevel1' OR parentgroup = '$aLevel2' OR itemid = $oneParentcd  OR parentgroup LIKE '$pGroup%'  and isentry=0 order by parentgroup, parentcd ";

        }else if($activitylevel==3){
          $aLevel1 =  substr($pGroup,0,13);
          $sql .=  "SELECT * FROM maindata WHERE  parentcd = 0 OR parentgroup = '$aLevel1' OR itemid = $oneParentcd  OR parentgroup LIKE '$pGroup%'  and isentry=0 order by parentgroup, parentcd ";

        }else{
          $sql .= "SELECT * FROM maindata WHERE  parentcd = 0 OR itemid = $oneParentcd  OR  parentgroup LIKE '$pGroup%'  and isentry=0 order by parentgroup, parentcd  ";
        }

    }else{
      $sql .= "SELECT * FROM maindata where   parentcd = 0 OR isentry=0  ";
    }
