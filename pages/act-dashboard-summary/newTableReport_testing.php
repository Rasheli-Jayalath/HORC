...
$ObjKfiDash12 = new ActDashboard();
$ObjKfiDash13 = new ActDashboard();
...
if(isset($itemids)&&$itemids!=0&&$itemids!='')
{
 ...
 $itemidcount = explode("_", $parentgroup);
    
     foreach ($itemidcount as $itemid) {
      $query2 = "Select itemname,itemid from maindata where itemid=$itemid";
      $objDb1-> dbQuery($query2);
      $rCount2 = $objDb1->totalRecords();
      if($rCount2 > 0){
        $res2=$objDb1->dbFetchArray();
        
        if ($itemname=="") {
          $itemname= '<a href="#" class="'.$btncolor.'"'.' onclick="reportgenButton('."'".$res2['itemid']."','". $res2['itemname']."'".')">'.$res2['itemname'].'</a>';
        }else{
          $itemname.= " &raquo; ".'<a href="#" class="'.$btncolor.'"'.' onclick="reportgenButton('."'".$res2['itemid']."','". $res2['itemname']."'".')">'.$res2['itemname'].'</a>';
        }
...
$sql="SELECT max(progressdate) as max_pdate FROM progress";
	$objDb2->dbQuery($sql);
	 $iiCount2 = $objDb2->totalRecords();
	 if($iiCount2>0)
	 {
		$pcdrows = $objDb2->dbFetchArray();
		$max_pdate=$pcdrows["max_pdate"];
...
<!--  <th width="60" rowspan="2">Weight</th>-->
  ...
  <th width="57" rowspan="2" style="word-break:break-all;">Cumulative Planned<br/><br />Progress to Date<br /><br />
  <?php if(isset($max_pdate)&& $max_pdate!='' && $max_pdate!=NULL && $max_pdate!='1970-01-01')
			{?><?php echo date('Y-m-d',strtotime($max_pdate));?><?php }?></th>
           
  <th width="63" rowspan="2" nowrap="nowrap">Cumulative Actual<br /><br /> Progress to Date<br /><br /> <?php if(isset($max_pdate)&& $max_pdate!='' && $max_pdate!=NULL && $max_pdate!='1970-01-01')
			{?><?php echo date('Y-m-d',strtotime($max_pdate));?><?php }?></th>
 <?php 
 if(isset($progress_month)&& $progress_month!='' && $progress_month!=NULL && $progress_month!='1970-01-01')
			{?>
            <th width="63" rowspan="2" nowrap="nowrap">Planned Progress<br /><br /> For Month<br /><br />(<?php echo date('Y-m-d',strtotime($progress_month));?>)</th>
 <?php }?>  
 <?php if(isset($progress_month)&& $progress_month!='' && $progress_month!=NULL && $progress_month!='1970-01-01')
			{?>
            <th width="63" rowspan="2" nowrap="nowrap">Actual Progress<br /><br /> For Month<br /><br />(<?php echo date('Y-m-d',strtotime($progress_month));?>)</th>
 <?php }?>  
</tr>
<tr>
...
<?php if($parentgroup!=0&&$parentgroup!="")
		...
		if($check==0)
		{
		  $reportquery_act ="SELECT * from maindata where itemid=".$itemids;
		 $ObjKfiDash1->dbQuery($reportquery_act);
		}
	while ($reportdata_act = $ObjKfiDash1->dbFetchArray()) {
		...
		$act_itemid=$reportdata_act["itemid"];
   ...
   $reportquery ="select startdate , enddate, baseline, progressmonth, total_progress, required_rate, actual_rate, expected_date, total_days, days_elapsed, isdelay, isdelayed From activity  where itemid=".$reportdata_act["itemid"];
	 ...
	 $reportquery ="select min(b.startdate) as startdate , max(b.enddate) as enddate, sum(b.baseline) as baseline,max(b.progressmonth) as progressmonth, sum(b.total_progress) as total_progress, b.required_rate, b.actual_rate, b.expected_date, b.total_days, b.days_elapsed, b.isdelay, b.isdelayed  From activity b where itemid=".$reportdata_act["itemid"];
		...
		else
		{
			...
			if($itemids==1)
			{
				$ObjKfiDash22->GetPlannedQtysOutputLevelMain($reportdata_act["parentgroup"],$reportdata_act["itemid"],$current_date_c,$reportdata['enddate'],$reportdata['startdate'],$reportdata_act["isentry"]);
			...
			if(isset($progress_month)&& $progress_month!='' && $progress_month!=NULL && $progress_month!='1970-01-01')
			{
				 $monthly_planned_progress=$ObjKfiDash12->MonthlyPlannedProgressOutput($reportdata_act["itemid"],$progress_month,$reportdata_act["isentry"]);
				  $monthly_actual_progress=$ObjKfiDash13->MonthlyActualProgressOutput($reportdata_act["itemid"],$progress_month,$reportdata_act["isentry"]);
...
<td style="text-align:left;"><a href="#"class="<?php echo $btncolor ?>" onclick="reportgenButton('<?php echo $act_itemid; ?>','<?php echo $act_name; ?>')"><span style="text-align:center;"><?php echo $act_code;?></span></a></td>
...
<td style="text-align:right;"><?php if($reportdata["baseline"]!=0){
	echo number_format($planned_progress/$reportdata["baseline"]*100,2) ." %";
}
else
{
echo "0.00 %";}?></td>
...
<?php if(isset($progress_month)&& $progress_month!='' && $progress_month!=NULL && $progress_month!='1970-01-01')
			{?>
            <td align="right"><?php echo number_format($monthly_planned_progress,2);?>&nbsp;%</td>
 <?php }?> 
 <?php if(isset($progress_month)&& $progress_month!='' && $progress_month!=NULL && $progress_month!='1970-01-01')
			{?>
            <td align="right"><?php echo number_format($monthly_actual_progress,2);?>&nbsp;%</td>
 <?php }?> 
</tr>
<?php
$numberDays=0;
...
<?php 
            ...
            <tr >
<td height="20" style="text-align:left;border-bottom-color:#FFF;background-color: rgb(169, 210, 248);" colspan="10">
