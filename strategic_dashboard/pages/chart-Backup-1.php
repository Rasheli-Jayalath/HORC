...
if (!isset($_REQUEST['back'])) {
	if (isset($_SESSION['srid'])) {
		...
		$plistresult = mysqli_query($db, $plistsql);
		
		while ($plistdata = mysqli_fetch_array($plistresult)) {
		...
		$projSQL = "select max(pid) as pid from t002project";
	  	$projSQLResult = mysqli_query($db, $projSQL);
...
$chartSQLResultd = mysqli_query($db, $chartSQLD);
...
$chartSQLResult = mysqli_query($db, $chartSQL);
...
$funSQLResult1 = mysqli_query($db, $funSQL1);
...
$issueSQLResult = mysqli_query($db, $issueSQL);
?>  
